...
class BC17():
    ...
    def add_table_Analysis_of_Somatic_Variant_Genes_and_Loci(self):  # 模版中 《体细胞变异基因及位点解析》《重要靶向药物小结》之间是没有分页符的。
        def Add_clinical_significance_tips_to_sub_table(row_index) -> str:  # 此方法会在 add_sub_table 方法内部被调用。
            df_temp = pd.read_csv(f'/refhub/ref/matching/{self.sample}注释匹配表.csv',header=None,names=['Genetic_variation','Citations'],sep=':') # 读前面add_table_Targeted_treatment_tips生成的中间表。
...
if __name__ == '__main__':
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            ...
            doc.save(f'/archive/word/{sample_temp}.docx')
