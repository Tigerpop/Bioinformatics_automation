...
class explain():   # 解读 c.96+1G>T 以及 p.L747_P753delinsS 的意思。
    def __init__(self):
        ...
        Amino_acid_list = ['甘氨酸','丙氨酸','缬氨酸','亮氨酸','异亮氨酸','脯氨酸','苯丙氨酸','酪氨酸','色氨酸','丝氨酸','苏氨酸','半胱氨酸','蛋氨酸','天冬酰胺','谷氨酰胺','天冬氨酸','谷氨酸','赖氨酸','精氨酸','组氨酸']
        abbreviate_table = ['G','A','V','L','I','P','F','Y','W','S','T','C','M','N','Q','D','E','K','R','H']
    ...
    def Interpretative_p(self,sentence):
        ...
        if len(match_group_list) > 1:
            self.position = '第'+match_group_list[0][1:]+'位'+self.Amino_acid_dict[match_group_list[0][0]]+'至'+'第'+match_group_list[1][1:]+'位'+self.Amino_acid_dict[match_group_list[1][0]]
...
class BC17():
    ...
    def __init__(self,sample):
        self.sample = sample                      # 模版传入的样本编号。
        # self.BC17_received = pd.read_csv('BC17_received.csv', sep='\t')
        df = pd.read_excel('2022WSSW005790-T.summary.xlsx',sheet_name=None)
        self.BC17_meta,self.BC17_snpindel,self.BC17_fusion,self.BC17_cnv,self.BC17_qc = df['meta'],df['snpindel'],df['fusion'],df['cnv'],df['qc']
        print(self.BC17_meta['name'].iloc[0])
        df = pd.read_excel('DrugApproval.xlsx', sheet_name=None)
        self.DrugApproval = df['药物获批情况表'].dropna(subset=['ApprovedContent'], how='any')
        df = pd.read_excel('GeneInfo_20230216.xlsx',sheet_name=None)
        self.GeneIndo = df['Genename_table'].dropna(subset=['Genename'], how='any')
        self.doc = Document('None.docx')
        self.Citation = pd.read_excel('./citation_20230322.xlsx',sheet_name=None)['citation']
    ...
    def fixed_position(self,End_character):                             #   End_character 用类似于 'pathogenictable'。
        for paragraph in self.doc.paragraphs:
            ...
            if paragraph_text.endswith(End_character):                  # 定位目标段落。
                target = paragraph
                return target
    def format_Cell(self,table,position=[(0,0)],size=10,color=RGBColor(0, 0, 0),left=False):       # doxc 中 table 和 table 中的cell 都是可变对象。
    ...
    def format_Table(self,table,header_size=10,col_width=[1,1],header_color=RGBColor(0, 0, 0),Border=[],Border_all=False):
        # 遍历每一行
    ...
    def machining_table_0(self,num_th=0):                               # 给第0个表格差值。
        ...
        table.cell(2, 1).text = self.BC17_meta['到样日期*'].iloc[0]
        table.cell(2, 3).text = self.BC17_meta['送检医院'].iloc[0]
        ...
        table.cell(5, 1).text = '-'
        # 改字体。
        self.format_Cell(table,position=[(1,1),(1,3),(1,5),(2,1),(2,3),(3,1),(3,3),(4,1),(4,3),(5,1)],size=10)
        return self.doc
    ...
    # 第一先考虑snpindel 这个sheet 中的s1 情况。
        ...
        for index,row in df_ref.iterrows():
            ...
            if row['Gene.smallrefGene']=='EGFR':
                if row['EXON']=='exon18': self.cell['cell_5'] = self.cell['cell_5'] + str(row['AAchange'])+f" ({row['VAF'].split(',')[0]})" + "\n"
                elif row['EXON'] == 'exon19': self.cell['cell_6'] = self.cell['cell_6'] + str(row['AAchange'])+f" ({row['VAF'].split(',')[0]})" + "\n"
                elif row['EXON'] == 'exon20': self.cell['cell_7'] = self.cell['cell_7'] + str(row['AAchange'])+f" ({row['VAF'].split(',')[0]})" + "\n"
                elif row['EXON'] == 'exon21': self.cell['cell_8'] = self.cell['cell_8'] + str(row['AAchange'])+f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] =='ALK': self.cell['cell_1'] = self.cell['cell_1'] + str(row['AAchange'])+f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'ATK1': self.cell['cell_2'] = self.cell['cell_2'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'BRAF': self.cell['cell_3'] = self.cell['cell_3'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'BCL2L11': self.cell['cell_4'] = self.cell['cell_4'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'ERBB2': self.cell['cell_9'] = self.cell['cell_9'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'KRAS': self.cell['cell_10'] = self.cell['cell_10'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'MAP2K1': self.cell['cell_11'] = self.cell['cell_11'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'MET': self.cell['cell_12'] = self.cell['cell_12'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'PIK3CA': self.cell['cell_13'] = self.cell['cell_13'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'PTEN': self.cell['cell_14'] = self.cell['cell_14'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'NTRK1': self.cell['cell_15'] = self.cell['cell_15'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'NTRK2': self.cell['cell_16'] = self.cell['cell_16'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'NTRK3': self.cell['cell_17'] = self.cell['cell_17'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'NRAS': self.cell['cell_18'] = self.cell['cell_18'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'RET': self.cell['cell_19'] = self.cell['cell_19'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
            elif row['Gene.smallrefGene'] == 'ROS1': self.cell['cell_20'] = self.cell['cell_20'] + str(row['AAchange']) + f" ({row['VAF'].split(',')[0]})" + "\n"
    # 第二考虑 fusion 这个sheet 中的s1 情况。
        df_ref = self.BC17_fusion[self.BC17_fusion['review'].isin(['S1','S2'])]
        # print(df_ref)
        for index, row in df_ref.iterrows():
            for i in range(1,len(table.rows)):
                if table.cell(i, 0).paragraphs[0].text == row['Region1']: self.cell[f'cell_{i}'] = self.cell[f'cell_{i}'] + row['Region1']+'-'+row['Region2'] +" 基因融合\n"
                if table.cell(i, 0).paragraphs[0].text == row['Region2']: self.cell[f'cell_{i}'] = self.cell[f'cell_{i}'] + row['Region1']+'-'+row['Region2'] +" 基因融合\n"
    # 第三考虑 cnv 这个sheet 中的s1 情况。
        df_ref = self.BC17_cnv[self.BC17_cnv['review'].isin(['S1','S2'])]
        # print(df_ref)
        for index, row in df_ref.iterrows():
            for i in range(1,len(table.rows)):
                if table.cell(i, 0).paragraphs[0].text == row['Gene']: self.cell[f'cell_{i}'] = self.cell[f'cell_{i}'] +"基因扩增\n"
    ...
    # 建立表1。
        ...
        if len(self.cell)==0: # 空 情况。
            ...
            for index, row in df_ref.iterrows():
                ...
                table.cell(i, 6).paragraphs[0].text = '{:.2f}%'.format((float(row['Break_support1'])+float(row['Break_support2']))/float(row['Total_depth'])*100 )
            ...
            for index, row in df_ref.iterrows():
                ...
                table.cell(i, 1).paragraphs[0].text = self.GeneIndo[self.GeneIndo['Genename']==row['Gene']]['RNA'].iloc[0].split('.')[0]
    ...
    # 建立表2。
        ...
        if num_row == 0:     # 空情况。
            ...
            for index, row in df_ref_2.iterrows():
                ...
                table_1.cell(i, 6).paragraphs[0].text = '{:.2f}%'.format((float(row['Break_support1'])+float(row['Break_support2']))/float(row['Total_depth'])*100 )
                self.cell_1.append(table_1.cell(i, 0).paragraphs[0].text)
                i += 1
            for index, row in df_ref_3.iterrows():
                table_1.cell(i, 0).paragraphs[0].text = row['Gene']
                table_1.cell(i, 1).paragraphs[0].text = self.GeneIndo[self.GeneIndo['Genename'] == row['Gene']]['RSG'].iloc[0]
    ...
    # 构建靶向治疗提示表 的规则方法。
        def build_Rules(Genetic_variation,variation_type)-> (str, str, str,str):  # 一次处理一行。返回的是  # 本癌药物 、其它癌药物、其它癌药物、耐药药物。
            ...
            df = pd.read_excel('./VarLabel_Drug_Sensitivity_20230322.xlsx',sheet_name=None)
            ...
            if re.findall(r'基因融合',Genetic_variation)==[] and re.findall(r'基因扩增',Genetic_variation)==[]:
                ...
                df_result_0 = df_sensitive[(df_sensitive['Gene']==name)&(df_sensitive['VarLabel'].isin(alteration_list))]
                ...
                df_result_1 = df_sensitive[(df_sensitive['Gene']==name)&(df_sensitive['VarLabel'].astype(str).str.findall('\d+').apply(set).apply(lambda x: bool(x & set(alteration_1_list))))]
                ...
                alteration_0 = '融合'
                df_result_0 = df_sensitive[(df_sensitive['Gene'] == name_list[0]) & (df_sensitive['VarLabel'].isin([alteration_0]))]
                df_result_1 = df_sensitive[(df_sensitive['Gene'] == name_list[1]) & (df_sensitive['VarLabel'].isin([alteration_0]))]
                df_result = pd.concat([df_result_0,df_result_1])
            ...
            # 按照cnv 方法来
                ...
                name = ele[0]
                alteration_0 = '扩增'
                df_result = df_sensitive[(df_sensitive['Gene'] == name) & (df_sensitive['VarLabel'].isin([alteration_0]))]
            ...
            drug_list = []
            for items in drug_list_raw: drug_list.extend(items.split(',')) # ['奥希替尼', '伏美替尼', '厄洛替尼+贝伐珠单抗', '纳武单抗+厄洛替尼']
            ...
            for i,row in df_result.iterrows():
                ...
                for drug in row['Drug'].split(','):
                    ...
                    # 本癌药物,潜在耐药药物。(定义耐药药物是指，由这个变异搜到的药物匹配癌种对应相同，但是变异对药物有耐药性。)
                        ...
                        df_temp = self.DrugApproval[self.DrugApproval['DrugName'].isin([drug])]  # 这个 df_temp 是 drug approve表中的 子集。
                        # print(df_temp)
                        ...
                        if not df_temp[df_temp['OncoType'].astype(str).str.split(',').apply(set).apply(lambda x: bool(x & set(self.BC17_meta['cancer'].tolist())))].empty:
                            if row['Sensitivity']=='敏感':  self_drug += drug+','
                            if row['Sensitivity'] == '耐药': resistance_Drug += drug+','
                        # print('本癌药物: ', self_drug)
                    # 其它癌药物
                        if df_temp[df_temp['OncoType'].astype(str).str.split(',').apply(set).apply(lambda x: bool(x & set(self.BC17_meta['cancer'].tolist())))].empty:
            ...
            if len(Clinical_drugs) > 1: Clinical_drugs = '\n'.join(list(set(Clinical_drugs.strip('/').strip(',').split(','))))
            if len(resistance_Drug) > 1: resistance_Drug = '\n'.join(list(set(resistance_Drug.strip('/').strip(',').split(','))))
            # 以下为 是否生成 中间文件，可选。(第一次运行的时候要记得打开，生成的中间文件后续会用上。)
            # if not os.path.isfile('./药物注释匹配表.csv'):
            #     df_result.to_csv('./药物注释匹配表.csv', mode='a', index=False)
            # else:
            #     df_result.to_csv('./药物注释匹配表.csv', mode='a', header=False,index=False)
            # if not os.path.isfile('./注释匹配表.csv'):
            #     citation_list = []
            #     Citations = df_result['Citation'].tolist()
            #     for items in Citations: citation_list.extend(str(items).split(','))
            #     with open('./注释匹配表.csv','w')as f:
            #         f.write(Genetic_variation.replace('\n',' ')+':'+','.join(citation_list)+'\n')
            # else:
            #     citation_list = []
            #     Citations = df_result['Citation'].tolist()
            #     for items in Citations: citation_list.extend(str(items).split(','))
            #     with open('./注释匹配表.csv','a+')as f:
            #         f.write(Genetic_variation.replace('\n',' ')+':'+','.join(citation_list)+'\n')
            # print('self_drug:', self_drug, 'other_drug:', other_drug, 'Clinical_drugs:', Clinical_drugs,
    ...
    # 先从基因检测结果表中读出非 '野生型/未检出' 结果。
        ...
        print(self.cell)     # ['ALK EML4-ALK 基因融合', 'EGFR p.L747_E749del', 'EGFR p.T790M', 'ERBB2 基因扩增', 'MET 基因扩增', 'RET RET-IRX3 基因融合']
        if len(self.cell)!=0:
        # 建立表。
            ...
            for i, row in enumerate(target_table.rows):
                ...
                if i > 0:
                    ...
                    self_drug, other_drug, Clinical_drugs ,resistance_Drug = build_Rules(Genetic_variation,row.cells[5].text)
        ...
        # 插入表。
            ...
            self.format_Cell(table,position=position,size=10,color=RGBColor(0, 0, 0))
            # 改表格。
            self.format_Table(table,col_width=[3, 3.25, 3.25, 3.25, 3.25])
        else:
        # 建立表。
    ...
    def add_table_Analysis_of_Somatic_Variant_Genes_and_Loci(self):  # 模版中 《体细胞变异基因及位点解析》《重要靶向药物小结》之间是没有分页符的。
        def Add_clinical_significance_tips_to_sub_table(row_index) -> str:  # 此方法会在 add_sub_table 方法内部被调用。
            df_temp = pd.read_csv('./注释匹配表.csv',header=None,names=['Genetic_variation','Citations'],sep=':') # 读前面add_table_Targeted_treatment_tips生成的中间表。
            ...
            citation_list = [ i for i in citation_list if re.findall('[a-z]+',i)!=[]] + [i for i in citation_list if re.findall('[a-z]+',i)==[]]  # 把 带字母的 元素 提前。
            # print(citation_list)
            for citation in citation_list:
                # print(self.Citation[self.Citation['Citation'].astype(str) == citation.strip(' ')]['Description'])
                description_list.append(self.Citation[self.Citation['Citation'].astype(str) == citation.strip(' ')]['Description'].iloc[0])
        ...
        def add_sub_table(address=[0,1]):   # address[1] 代表 了 体细胞变异结果 表的第几行,1只是默认值，可以变的。
            ...
            for i in range(6):                        # 每个新的表下面再加上6行。
                ...
                if i == 1:
                    describe = self.GeneIndo[self.GeneIndo['Genename'] == self.table_analysis.cell(address[1], 0).paragraphs[0].text]['Describe']
                    # print(describe)
                    new_table.cell(i + 2, 0).paragraphs[0].text = describe.astype(str).iloc[0] if (not describe.empty) and (not describe.isna().any()) else '/'
                if i == 2:
            ...
            if address[1] == 1 and address[1] != len(self.cell):                      # 第一个子表 且不只有一个子表。
                ...
                paging_paragraph = next_paragraph.insert_paragraph_before('')#(f"{address[1]}'")  # 分页符段落没法定位，所以只能在插入的时候赶紧完成后续操作。
    ...
    # 通过匹配table的表头来定位table。
        for table in self.doc.tables:
            if len(table.columns)>=4 and table.cell(0,0).paragraphs[0].text=='质控内容' and table.cell(0,1).paragraphs[0].text=='质控参数':
                target_table = table
    # 修改表的内容。
        ...
        if 'HE结果' in self.BC17_meta.columns: # 假定 self.BC17_meta['HE结果'].iloc[0] 是个百分数。
            if float(self.BC17_meta['HE结果'].iloc[0].strip('%')) / 100.0 >= 0.1: self.cell['cell_1'] += f"{'{:.0%}'.format(float(self.BC17_meta['HE结果'].iloc[0].strip('%'))/100)}"
            else: self.cell['cell_1'] += '10%'
        else:
            if self.BC17_meta['样本类型*'].iloc[0]=='组织': self.cell['cell_1'] += '10%'
            else : self.cell['cell_1'] += '不适用'
        if 'DNA总量' in self.BC17_meta.columns:
            if self.BC17_meta['样本类型*'].iloc[0]=='组织':
                if self.BC17_meta['DNA总量'].iloc[0]>=100: self.cell['cell_2'] += f"{str(int(self.BC17_meta['DNA总量'].iloc[0]))}"
                else: self.cell['cell_2'] += f"100"
            else:
                if self.BC17_meta['DNA总量'].iloc[0]>=30: self.cell['cell_2'] += f"{str(int(self.BC17_meta['DNA总量'].iloc[0]))}"
                else: self.cell['cell_2'] += f"30"
        else:
            if self.BC17_meta['样本类型*'].iloc[0]=='组织': self.cell['cell_2'] += '100'
            else : self.cell['cell_2'] += '30'
        if '预文库总量' in self.BC17_meta.columns:
            if self.BC17_meta['DNA总量'].iloc[0] >= 500: self.cell['cell_4'] += f"{str(int(self.BC17_meta['预文库总量'].iloc[0]))}"
            else: self.cell['cell_4'] += '500'
        else: self.cell['cell_4'] += '500'
        if 'depth' in self.BC17_qc.columns:
            if self.BC17_meta['样本类型*'].iloc[0]=='组织':
                if self.BC17_qc['depth'].iloc[0] >= 1000: self.cell['cell_5'] += f"{str(int(self.BC17_qc['depth'].iloc[0]))}"
                else: self.cell['cell_5'] += '1000'
            else:
                if self.BC17_qc['depth'].iloc[0] >= 5000: self.cell['cell_5'] += f"{str(int(self.BC17_qc['depth'].iloc[0]))}"
                else: self.cell['cell_5'] += '5000'
        else:
            if self.BC17_meta['样本类型*'].iloc[0] == '组织': self.cell['cell_5'] += '1000'
            else: self.cell['cell_5'] += '5000'
        if '文库多样性' in self.BC17_meta.columns:
            if float(self.BC17_meta['文库多样性'].iloc[0].strip('%')) / 100.0 >= 0.1: self.cell['cell_6'] += f"{'{:.2%}'.format(float(self.BC17_meta['文库多样性'].iloc[0].strip('%'))/100)}"
            else: self.cell['cell_6'] += '10%'
        else: self.cell['cell_6'] += '10%'
        if self.BC17_qc['inser_size_peak'].iloc[0] <= 300: self.cell['cell_7'] += f"{str(int(self.BC17_qc['inser_size_peak'].iloc[0]))}"
        else: self.cell['cell_7'] += '200'
        if float(self.BC17_qc['coverage'].iloc[0].strip('%')) / 100.0 >= 0.9: self.cell['cell_8'] += f"{'{:.2%}'.format(float(self.BC17_qc['coverage'].iloc[0].strip('%'))/100)}"
        else: self.cell['cell_8'] += '90.05%'
        if float(self.BC17_qc['mapped'].iloc[0].strip('%')) / 100.0 >= 0.95: self.cell['cell_9'] += f"{'{:.2%}'.format(float(self.BC17_qc['mapped'].iloc[0].strip('%'))/100)}"
        else: self.cell['cell_9'] += '95.05%'
        if float(self.BC17_qc['base_quality'].iloc[0].strip('%')) / 100.0 >= 0.8: self.cell['cell_10'] += f"{'{:.2%}'.format(float(self.BC17_qc['base_quality'].iloc[0].strip('%'))/100)}"
        else: self.cell['cell_10'] += '80.05%'
    ...
    def add_header(self,manual_Date=None):  # 改页眉。
        ...
        header.tables[0].cell(0, 0).paragraphs[0].text = '清港泉生物科技有限公司 '
    ...
    def add_date(self,manual_Date=None):
        # 通过匹配table的表头来定位table。
        for table in self.doc.tables:
            if len(table.columns) == 2 and table.cell(0, 0).paragraphs[0].text == '检测人：' and table.cell(0, 1).paragraphs[0].text == '复核人：':
                target_table = table
