# 设置编码为 UTF-8
options(encoding = "UTF-8")

#  load the package
library(panelcn.mops)
data(panelcn.mops)

# Rscript my_script.R arg1_value arg2_value
# args <- commandArgs(trailingOnly = TRUE)
# arg1 <- args[1]
# arg2 <- args[2]
# my_function(arg1, arg2)
args <- commandArgs(trailingOnly = TRUE)
sample <- args[1]
sample_path <- args[2]
generate_location <- args[3]
bed_key <- args[4] 
print(sample)
print(bed_key)

BC17 <- function(sample,bed) {
  # 获取计数窗
  bed <- bed # "/refhub/hg19/target/BC17/panelcn.bed"
  countWindows <- getWindows(bed)
  # 获取read counts
  testbam <- sprintf("/home/chenyushao/py_streaming_generate/%s/%s.bwa_mem.bam", sample,sample)
  cat("testbam is :", testbam, "\n")
  # testbam <- "/home/chenyushao/py_streaming_generate/2022WSSW004338-T/2022WSSW004338-T.bwa_mem.bam"
  test <- countBamListInGRanges(countWindows = countWindows,bam.files = testbam, read.width = 150)
  # 可以和decon公用一套对照。
  controlbam1 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BC17/2022WSSW005762-T.bwa_mem.bam"
  control1 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam1, read.width = 150)
  controlbam2 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BC17/2022WSSW005763-T.bwa_mem.bam"
  control2 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam2, read.width = 150)
  controlbam3 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BC17/2022WSSW005766-T.bwa_mem.bam"
  control3 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam3, read.width = 150)
  controlbam4 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BC17/2022WSSW005767-T.bwa_mem.bam"
  control4 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam4, read.width = 150)
  controlbam5 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BC17/2022WSSW005768-T.bwa_mem.bam"
  control5 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam5, read.width = 150)
  controlbam6 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BC17/2022WSSW006018-T.bwa_mem.bam"
  control6 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam6, read.width = 150)
  controlbam7 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BC17/2022WSSW006024-T.bwa_mem.bam"
  control7 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam7, read.width = 150)
  controlbam8 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BC17/2022WSSW006040-T.bwa_mem.bam"
  control8 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam8, read.width = 150)
  controlbam9 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BC17/2022WSSW006081-T.bwa_mem.bam"
  control9 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam9, read.width = 150)
  controlbam10 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BC17/2022WSSW006082-T.bwa_mem.bam"
  control10 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam10, read.width = 150)
  # 运行算法
  XandCB <- test
  elementMetadata(XandCB) <- cbind(elementMetadata(XandCB),
                                   elementMetadata(control1),
                                   elementMetadata(control2),
                                   elementMetadata(control3),
                                   elementMetadata(control4),
                                   elementMetadata(control5),
                                   elementMetadata(control6),
                                   elementMetadata(control7),
                                   elementMetadata(control8),
                                   elementMetadata(control9),
                                   elementMetadata(control10)
                                   )
  resultlist <- runPanelcnMops(XandCB,
                               testiv = 1:ncol(elementMetadata(test)),
                               countWindows = countWindows)
  # 结果可视化
  sampleNames <- colnames(elementMetadata(test))
  resulttable <- createResultTable(resultlist = resultlist, XandCB = XandCB,
                                   countWindows = countWindows,
                                   selectedGenes = NULL,
                                   sampleNames = sampleNames)
  write.table(resulttable, "./panelcn_output.txt", sep="\t", row.names=FALSE)
}

Q120 <- function(sample,bed) {
  # 获取计数窗
  bed <- bed# "/refhub/hg19/target/Q120/panelcn.bed"
  countWindows <- getWindows(bed)
  # 获取read counts
  testbam <- sprintf("/home/chenyushao/py_streaming_generate/%s/%s.bwa_mem.bam", sample,sample)
  cat("testbam is :", testbam, "\n")
  # testbam <- "/home/chenyushao/py_streaming_generate/2022WSSW004338-T/2022WSSW004338-T.bwa_mem.bam"
  test <- countBamListInGRanges(countWindows = countWindows,bam.files = testbam, read.width = 150)
  # 可以和decon公用一套对照。
  controlbam1 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/Q120/2022WSSW000956-T.bwa_mem.bam"
  control1 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam1, read.width = 150)
  controlbam2 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/Q120/2022WSSW000961-T.bwa_mem.bam"
  control2 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam2, read.width = 150)
  controlbam3 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/Q120/2022WSSW002334-T.bwa_mem.bam"
  control3 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam3, read.width = 150)
  controlbam4 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/Q120/2022WSSW000308-T.bwa_mem.bam"
  control4 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam4, read.width = 150)
  controlbam5 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/Q120/2023WSSW000066-T.bwa_mem.bam"
  control5 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam5, read.width = 150)
  controlbam6 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/Q120/2022WSSW000997-T.bwa_mem.bam"
  control6 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam6, read.width = 150)
  controlbam7 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/Q120/2022WSSW000995-T.bwa_mem.bam"
  control7 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam7, read.width = 150)
  controlbam8 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/Q120/2022WSSW006061-T.bwa_mem.bam"
  control8 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam8, read.width = 150)
  controlbam9 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/Q120/2022WSSW005109-T.bwa_mem.bam"
  control9 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam9, read.width = 150)
  controlbam10 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/Q120/2022WSSW006057-T.bwa_mem.bam"
  control10 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam10, read.width = 150)
  # 运行算法
  XandCB <- test
  elementMetadata(XandCB) <- cbind(elementMetadata(XandCB),
                                   elementMetadata(control1),
                                   elementMetadata(control2),
                                   elementMetadata(control3),
                                   elementMetadata(control4),
                                   elementMetadata(control5),
                                   elementMetadata(control6),
                                   elementMetadata(control7),
                                   elementMetadata(control8),
                                   elementMetadata(control9),
                                   elementMetadata(control10)
  )
  resultlist <- runPanelcnMops(XandCB,
                               testiv = 1:ncol(elementMetadata(test)),
                               countWindows = countWindows)
  # 结果可视化
  sampleNames <- colnames(elementMetadata(test))
  resulttable <- createResultTable(resultlist = resultlist, XandCB = XandCB,
                                   countWindows = countWindows,
                                   selectedGenes = NULL,
                                   sampleNames = sampleNames)
  write.table(resulttable, "./panelcn_output.txt", sep="\t", row.names=FALSE)
}

NBC650 <- function(sample) {
  # 获取计数窗
  bed <- "/refhub/hg19/target/NBC650/panelcn.bed"
  countWindows <- getWindows(bed)
  # 获取read counts
  testbam <- sprintf("/home/chenyushao/py_streaming_generate/%s/%s.bwa_mem.bam", sample,sample)
  cat("testbam is :", testbam, "\n")
  # testbam <- "/home/chenyushao/py_streaming_generate/2022WSSW004338-T/2022WSSW004338-T.bwa_mem.bam"
  test <- countBamListInGRanges(countWindows = countWindows,bam.files = testbam, read.width = 150)
  # 可以和decon公用一套对照。
  controlbam1 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/NBC650/2022WSSW004654-T.bwa_mem.bam"
  control1 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam1, read.width = 150)
  controlbam2 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/NBC650/2022WSSW005125-T.bwa_mem.bam"
  control2 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam2, read.width = 150)
  controlbam3 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/NBC650/2022WSSW004656-T.bwa_mem.bam"
  control3 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam3, read.width = 150)
  controlbam4 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/NBC650/2021WSSW010388-T.bwa_mem.bam"
  control4 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam4, read.width = 150)
  controlbam5 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/NBC650/2022WSSW004677-T.bwa_mem.bam"
  control5 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam5, read.width = 150)
  controlbam6 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/NBC650/2022WSSW004116-T.bwa_mem.bam"
  control6 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam6, read.width = 150)
  controlbam7 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/NBC650/2022WSSW005590-T.bwa_mem.bam"
  control7 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam7, read.width = 150)
  controlbam8 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/NBC650/2022WSSW004157-T.bwa_mem.bam"
  control8 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam8, read.width = 150)
 # 运行算法
  XandCB <- test
  elementMetadata(XandCB) <- cbind(elementMetadata(XandCB),
                                   elementMetadata(control1),
                                   elementMetadata(control2),
                                   elementMetadata(control3),
                                   elementMetadata(control4),
                                   elementMetadata(control5),
                                   elementMetadata(control6),
                                   elementMetadata(control7),
                                   elementMetadata(control8)
  )
  resultlist <- runPanelcnMops(XandCB,
                               testiv = 1:ncol(elementMetadata(test)),
                               countWindows = countWindows)
  # 结果可视化
  sampleNames <- colnames(elementMetadata(test))
  resulttable <- createResultTable(resultlist = resultlist, XandCB = XandCB,
                                   countWindows = countWindows,
                                   selectedGenes = NULL,
                                   sampleNames = sampleNames)
  write.table(resulttable, "./panelcn_output.txt", sep="\t", row.names=FALSE)
}

BCP650 <- function(sample) {
  # 获取计数窗
  bed <- "/refhub/hg19/target/BCP650/panelcn.bed"
  countWindows <- getWindows(bed)
  # 获取read counts
  testbam <- sprintf("/home/chenyushao/py_streaming_generate/%s/%s.bwa_mem.bam", sample,sample)
  cat("testbam is :", testbam, "\n")
  # testbam <- "/home/chenyushao/py_streaming_generate/2022WSSW004338-T/2022WSSW004338-T.bwa_mem.bam"
  test <- countBamListInGRanges(countWindows = countWindows,bam.files = testbam, read.width = 150)
  # 可以和decon公用一套对照。
  controlbam1 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BCP650/2022WSSW000306-T.bwa_mem.bam"
  control1 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam1, read.width = 150)
  controlbam2 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BCP650/2023WSSW000103-T.bwa_mem.bam"
  control2 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam2, read.width = 150)
  controlbam3 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BCP650/2023WSSW000052-T.bwa_mem.bam"
  control3 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam3, read.width = 150)
  controlbam4 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BCP650/2023WSSW000064-T.bwa_mem.bam"
  control4 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam4, read.width = 150)
  controlbam5 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BCP650/2023WSSW000062-T.bwa_mem.bam"
  control5 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam5, read.width = 150)
  controlbam6 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BCP650/2023WSSW000659-T.bwa_mem.bam"
  control6 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam6, read.width = 150)
  controlbam7 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BCP650/2022WSSW005609-T.bwa_mem.bam"
  control7 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam7, read.width = 150)
  controlbam8 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/BCP650/2023WSSW000057-T.bwa_mem.bam"
  control8 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam8, read.width = 150)
  # 运行算法
  XandCB <- test
  elementMetadata(XandCB) <- cbind(elementMetadata(XandCB),
                                   elementMetadata(control1),
                                   elementMetadata(control2),
                                   elementMetadata(control3),
                                   elementMetadata(control4),
                                   elementMetadata(control5),
                                   elementMetadata(control6),
                                   elementMetadata(control7),
                                   elementMetadata(control8)
  )
  resultlist <- runPanelcnMops(XandCB,
                               testiv = 1:ncol(elementMetadata(test)),
                               countWindows = countWindows)
  # 结果可视化
  sampleNames <- colnames(elementMetadata(test))
  resulttable <- createResultTable(resultlist = resultlist, XandCB = XandCB,
                                   countWindows = countWindows,
                                   selectedGenes = NULL,
                                   sampleNames = sampleNames)
  write.table(resulttable, "./panelcn_output.txt", sep="\t", row.names=FALSE)
}

SD160 <- function(sample,bed) {
  # 获取计数窗
  bed <- bed # "/refhub/hg19/target/SD160/panelcn.bed"
  countWindows <- getWindows(bed)
  # 获取read counts
  testbam <- sprintf("/home/chenyushao/py_streaming_generate/%s/%s.bwa_mem.bam", sample,sample)
  cat("testbam is :", testbam, "\n")
  # testbam <- "/home/chenyushao/py_streaming_generate/2022WSSW004338-T/2022WSSW004338-T.bwa_mem.bam"
  test <- countBamListInGRanges(countWindows = countWindows,bam.files = testbam, read.width = 150)
  # 可以和decon公用一套对照。
  controlbam1 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/SD160/2022WSSW006085-T.bwa_mem.bam"
  control1 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam1, read.width = 150)
  controlbam2 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/SD160/2022WSSW002335-T.bwa_mem.bam"
  control2 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam2, read.width = 150)
  controlbam3 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/SD160/2022WSSW005894-T.bwa_mem.bam"
  control3 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam3, read.width = 150)
  controlbam4 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/SD160/2023WSSW000662-T.bwa_mem.bam"
  control4 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam4, read.width = 150)
  controlbam5 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/SD160/2022WSSW001563-T.bwa_mem.bam"
  control5 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam5, read.width = 150)
  controlbam6 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/SD160/2022WSSW000976-T.bwa_mem.bam"
  control6 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam6, read.width = 150)
  controlbam7 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/SD160/2023WSSW000029-T.bwa_mem.bam"
  control7 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam7, read.width = 150)
  controlbam8 <- "/refhub/hg19/toolbox_and_RefFile/Decon_ref_bam_file_txt/SD160/2022WSSW005858-T.bwa_mem.bam"
  control8 <- countBamListInGRanges(countWindows = countWindows,bam.files = controlbam8, read.width = 150)
  # 运行算法
  XandCB <- test
  elementMetadata(XandCB) <- cbind(elementMetadata(XandCB),
                                   elementMetadata(control1),
                                   elementMetadata(control2),
                                   elementMetadata(control3),
                                   elementMetadata(control4),
                                   elementMetadata(control5),
                                   elementMetadata(control6),
                                   elementMetadata(control7),
                                   elementMetadata(control8)
  )
  resultlist <- runPanelcnMops(XandCB,
                               testiv = 1:ncol(elementMetadata(test)),
                               countWindows = countWindows)
  # 结果可视化
  sampleNames <- colnames(elementMetadata(test))
  resulttable <- createResultTable(resultlist = resultlist, XandCB = XandCB,
                                   countWindows = countWindows,
                                   selectedGenes = NULL,
                                   sampleNames = sampleNames)
  write.table(resulttable, "./panelcn_output.txt", sep="\t", row.names=FALSE)
}


# 
tmp_dir <- paste(generate_location, sample_path, "panelcn_generate", sep="/")
if (!dir.exists(tmp_dir)) {
  dir.create(tmp_dir)
}
setwd(tmp_dir)
if ( bed_key == "BC17T" ) {
  BC17(sample,bed="/refhub/hg19/target/BC17T/panelcn.bed") 
} else if ( bed_key == "BC17B" ) {
  BC17(sample,bed="/refhub/hg19/target/BC17B/panelcn.bed") 
} else if ( bed_key == "Q120T" | bed_key == "Q80T" | bed_key == "Q110T") {
  Q120(sample,bed="/refhub/hg19/target/Q120T/panelcn.bed")
} else if ( bed_key == "Q120B" | bed_key == "Q80B" | bed_key == "Q110B") {
  Q120(sample,bed="/refhub/hg19/target/Q120B/panelcn.bed")
} else if ( bed_key == "NBC650") {
  NBC650(sample)
} else if ( bed_key == "BCP650") {
  BCP650(sample)
} else if ( bed_key == "SD160T") {
  SD160(sample,bed="/refhub/hg19/target/SD160T/panelcn.bed")
} else if ( bed_key == "SD160B") {
  SD160(sample,bed="/refhub/hg19/target/SD160B/panelcn.bed")
} else {
  print('panel is not exitst.')
}

