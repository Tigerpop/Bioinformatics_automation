...
def shell_func_0(i,queue,ref_fasta=ref_fasta,chr_split=chr_split,sample=sample,split_vcf=split_vcf):
    ...
    if i == "7_replenish":     # 对于补充处理的关键区域单独call,并且缩小颗粒度。
        ...
        with open(f'{log_path}/callMutation_normal.log', 'a+') as f0,open(f'{log_path}/callMutation_debug.log', 'a+') as f1:
            if out is None:
                f0.write(f"")
            else:
...
class split_callMutation_merge():
    ...
    def callMutation(self):  
        ...
        os.chdir("chr_split")
        shutil.copyfile(f'{chr_split}/{sample}.markdup.REF_chr7.bam', f'{chr_split}/{sample}.markdup.REF_chr7_replenish.bam')
        shutil.copyfile(f'{chr_split}/{sample}.markdup.REF_chr7.bam.bai', f'{chr_split}/{sample}.markdup.REF_chr7_replenish.bam.bai')
