...
def download_checkMD5(oss,sample):
    ...
    with open(f'{temp_download}/temp_{oss_body}.txt','r')as f0:
        for line in f0:
            MD5_line = re.findall('MD5',line)
            if MD5_line != []:
                MD5_oss_url = line.split(' ')[-1].strip()
                print('MD5_oss_url is :',MD5_oss_url)
                cmd_1 = f"ossutil64 cp {MD5_oss_url} {temp_download}/MD5_{oss_body}.txt -c {config}"
            RawData_line = re.findall(r'.*/(RawData|00.mergeRawFq)/$',line)
            if RawData_line != []:
                ...
                cmd_2 = f"ossutil64 cp -r {RawData_url} {temp_download}/{RawData}/ -c {config} --include '*{sample}*' "
...
def read_received_new_table():
    ...
    for index, row in df.iterrows():
        ...
        if result:
            ...
            p.communicate()
            # 文件夹结构不包括文件，写入被监控文件夹。
            if os.path.exists(f'/home/chenyushao/py_streaming_generate/monitor/{sample}'): # 重复文件处理。移动进被监控文件夹之前还要这样再来一遍。 
                Time = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
                os.rename(f'/home/chenyushao/py_streaming_generate/monitor/{sample}',f'/home/chenyushao/py_streaming_generate/monitor/{sample}_before{Time}')
