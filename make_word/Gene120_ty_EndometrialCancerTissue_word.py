...
class Q110():
    def __init__(self, input_file):
        self.doc = Document('/refhub/ref/masterplate/120基因检测报告_通用_组织版_子宫内膜癌.docx')
    ...
    position,position_1 = [],[]
    @Tools_Decorator(tool='format_Cell', table_index=8,position=position, size=10, color=RGBColor(255, 0, 0))
    @Tools_Decorator(tool='format_Cell', table_index=8,position=position_1, size=10, color=RGBColor(0, 0, 0))
    @log_to_file(folder='./', filename='1.log')
    def Details_of_genetic_testing_results(self, position=[],position_1=[]):  # 给第个表格插入值。
        for temp_table in self.doc.tables:
            if len(temp_table.columns) == 3 \
                and temp_table.cell(0, 1).text == '检测内容' \
                and temp_table.cell(0, 2).text == '检测结果（仅显示致病/可能致病变异）' :
                target_table = temp_table
                break
        table = target_table
        print('检查点,基因检测结果详情表一共是{}列'.format(len(self.doc.tables[4].columns)))
        print(self.doc.tables[4].cell(0,0).text,self.doc.tables[4].cell(0,1).text,self.doc.tables[4].cell(0,2).text,self.doc.tables[4].cell(0,3).text,self.doc.tables[4].cell(0,4).text,)
        print(self.doc.tables[4].cell(1, 0).text,'/', self.doc.tables[4].cell(1, 1).text,'/',
              self.doc.tables[4].cell(1, 2).text,'/', self.doc.tables[4].cell(1, 3).text,'/',
              self.doc.tables[4].cell(1, 4).text, )
        print(self.doc.tables[4].cell(2, 0).text,'/', self.doc.tables[4].cell(2, 1).text,'/',
              self.doc.tables[4].cell(2, 2).text,'/', self.doc.tables[4].cell(2, 3).text,'/',
              self.doc.tables[4].cell(2, 4).text, )
    ...
    @Tools_Decorator(tool='format_Cell', position=position, table_index=4,size=10, color=RGBColor(0, 0, 0),left=True)
    ...
    @Tools_Decorator(tool='format_Cell', table_index=3,position=position, size=10, color=RGBColor(0, 0, 0))
