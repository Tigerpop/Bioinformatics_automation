        ...
        for i, row in enumerate(table.rows):
            ...
            for index,cell in enumerate(cells):
                ...
                if i == 0:
                    ...
                    self.Set_Background_Color(cell, "297EBE")
...
class Q80():
    def __init__(self, input_file):
        self.doc = Document('/refhub/ref/masterplate/55基因检测报告_中科_肺癌_测试_组织版.docx')
    ...
    position=[]
    @Tools_Decorator(tool='format_Cell',table_index=1,position = position  ,size=10) # 改字体.
    ...
    @Tools_Decorator(tool='format_Cell', table_index=-6,position=position, size=10, color=RGBColor(0, 0, 0))
    ...
    def machining_table_1(self, num_th=2):
        table = self.doc.tables[num_th]
        if len(self.cell)!=1 :
            variance_num = len(self.cell)
        elif self.cell[0].variant_type!='野生型':
    ...
    def Chemotherapy_drug_testing_0(self,position=[]):
        ...
        if total_rows % 2 == 0:
            ...
            for cell in new_row:
                cell.text = '-'
        # # 以下为分栏 测试
        target_section = section = self.doc.sections[4]  # 在母板中chemotable前后加上两个 分节符。
    ...
    position=[]
    #@Tools_Decorator(tool='format_Cell',table_index=0,position = position  ,size=12) # 改字体.
    def Write_table_0(self, position=[]):
        table = self.doc.tables[0]
        table.cell(0, 1).text = self.sample
        table.cell(1, 1).text = self.arrival_date
        table.cell(2, 1).text = self.name
        table.cell(3, 1).text = self.gender
        table.cell(4, 1).text = self.age
        temp_position = [(0, 1), (1, 1), (2, 1), (3, 1), (4, 1),(0, 0), (1, 0), (2, 0), (3, 0), (4, 0)]
        position.extend(temp_position)
        return self.doc
    ...
    #     return self.doc
    def add_header(self, manual_Date=None):  # 改页眉。
        section = self.doc.sections[3]  # 注意以下这里，如果 模板内前面加入几个分节符，可能回导致这里需要相应的修改。
        # 设置节的页面布局为奇偶页不同
        ...
        # header.tables[0].cell(0, 0).paragraphs[0].text = '上海解码医学检验所'  # '清港泉生物科技有限公司 ' 上海中科润达精准医学检验有限公司
        data_ = f'{datetime.datetime.now().date().strftime("%Y-%m-%d")}' if manual_Date == None else manual_Date
        sample_id, sample_name = self.BC17_meta['id'].iloc[0], self.BC17_meta['name'].iloc[0]
        odd_page_header.tables[0].cell(0, 1).paragraphs[0].text = sample_id.replace('-T', '').replace('-N','') + ' | ' + sample_name + ' | ' + data_
        odd_page_header.tables[0].cell(0, 0).paragraphs[0].text = '上海中科润达精准医学检验有限公司'
        # 获取偶数页的页眉，在偶数页的页眉中添加内容
        even_page_header = section.even_page_header
        # even_page_header.text = "偶数页页眉"
        even_page_header.tables[0].cell(0, 0).paragraphs[0].text = sample_id.replace('-T', '').replace('-N','') + ' | ' + sample_name + ' | ' + data_
        even_page_header.tables[0].cell(0, 1).paragraphs[0].text = '上海中科润达精准医学检验有限公司'

        even_page_header.tables[0].cell(0, 0).paragraphs[0].runs[0].font.color.rgb = RGBColor(41, 126, 190)
        even_page_header.tables[0].cell(0, 1).paragraphs[0].runs[0].font.color.rgb = RGBColor(41, 126, 190)
        odd_page_header.tables[0].cell(0, 1).paragraphs[0].runs[0].font.color.rgb = RGBColor(41, 126, 190)
        odd_page_header.tables[0].cell(0, 0).paragraphs[0].runs[0].font.color.rgb = RGBColor(41, 126, 190)
        return self.doc
...
if __name__ == '__main__':
    ...
    doc = q80.Write_table_0(position=[])
