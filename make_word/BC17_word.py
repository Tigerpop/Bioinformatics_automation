...
class BC17():
    def __init__(self, input_file):
        self.doc = Document('/refhub/ref/masterplate/解码17基因报告母版.2023v1的副本.docx')
        self.input_file = input_file
        self.sample = self.input_file.split('/')[-1].replace('.summary.xlsx', '')
        print(self.sample, 'input_file is : ', input_file)
        df = pd.read_excel(input_file, sheet_name=None)
        self.BC17_meta, self.BC17_fusion, self.BC17_cnv, self.BC17_qc = df['meta'].fillna(''), df['fusion'], df['cnv'], df['qc']
        ...
        self.Drug_order = ['吉非替尼','厄洛替尼','达克替尼','阿法替尼','埃克替尼','奥希替尼','阿美替尼','伏美替尼','厄洛替尼+雷莫芦单抗','厄洛替尼+贝伐珠单抗'] # 药物指定用这个顺序排序；
    ...
    position,position_1 = [],[]
    @Tools_Decorator(tool='format_Cell', table_index=2,position=position, size=10, color=RGBColor(255, 0, 0))
    @Tools_Decorator(tool='format_Cell', table_index=2,position=position_1, size=10, color=RGBColor(0, 0, 0))
    @log_to_file(folder='./', filename='1.log')
    def Details_of_genetic_testing_results(self, position=[],position_1=[]):  # 给第个表格插入值。
        ...
        print('self.cell 中的原始最后保留的只有：',reserve_index_list)
        ...
        if self.cell == [] and self.cancer == '肠癌':
            ele = self.Mutation_ele(gene='KRAS/NRAS/BRAF', rna='', nucleotide='' \
                , amino_acid='', exon='',variant_type='野生型', vaf_cnv='')
            self.cell.append(ele)
        print('self.cell 中最后里面有：',[ele.gene+' '+ele.variant_type for ele in self.cell])
        return self.doc
    ...
    def Analysis_of_Somatic_Variant_Genes_and_Loci(self,position=[]):  # 模版中 《体细胞变异基因及位点解析》《靶向药物注释》之间是没有分页符的。
        ...
        def insert_add_sub_table(i): # new_table 直接用最后一个 table。
            ...
            if address[1] == 1 and address[1] != len(self.cell):  # 第一个子表 且不只有一个子表。
                ...
                next_paragraph = self.class_Tools.fixed_position(self.doc,End_character='重要靶向药物小结')
...
if __name__ == '__main__':
    # 2023WSSW001974-T 2023WSSW001975-T 2023WSSW001529-T
    # sample = '2023WSSW001705-T'
    sample = sys.argv[1]
    date = sys.argv[2]
    word_name = sys.argv[3]
    print('sample',sample,'date',date,'word_name',word_name)
    bc17 = BC17(f'/archive/{date}/{sample}.summary.xlsx') #
    doc = bc17.sample_information(position=[])
    doc = bc17.Details_of_genetic_testing_results(position=[],position_1=[]) # 这一步其实对self.cell做了一次过滤。
    doc = bc17.Somatic_variation_results_0(position=[])
    doc = bc17.Somatic_variation_results_1(position=[])
    doc = bc17.targeted_Therapy_Tips(position=[])
    doc = bc17.Analysis_of_Somatic_Variant_Genes_and_Loci(position=[])
    doc = bc17.quality_Control_Results(position=[])
    doc = bc17.machining_table_1()

    # doc = bc17.Chemotherapy_drug_testing_1(position=[])
    # doc = bc17.Chemotherapy_drug_testing_0(position=[])
    # doc = bc17.Evaluation_of_therapeutic_effect_of_chemotherapy_drugs( position=[])
    # doc = bc17.MSI_detection(position=[])
    # doc = bc17.Evaluation_of_the_therapeutic_effect_of_immunotherapy(position=[])
    # doc = bc17.Targeted_drug_annotations(position=[])
    ...
    bc17.doc.save(f'/archive/word/{date}/{word_name}.docx')
